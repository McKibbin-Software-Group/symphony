    ...
    for module in modules.modules:
        for declaration in module.declarations:
            if declaration is None:
                logging.debug(f"Skipping None declaration in module {module.name}")
                exit("None declaration found")
            if not hasattr(declaration, "label") and not hasattr(declaration, "name"):
                logging.debug(
                    f"Skipping declaration without label or name: {declaration}"
                )
                exit("no label")

            if not hasattr(declaration.label, "value"):
                logging.debug(type(declaration))
                logging.debug(
                    f"Skipping declaration with invalid label: {declaration.label}"
                )
                exit("no label value")
            label: str = (
                f"{declaration.label.value}" if hasattr(declaration, "label") else ""
            )
...
def summarise_abstract_syntax_tree(ast_loader_result: ASTLoaderResult) -> str:
    ...
    result: str = abstract_syntax_tree_summary(
        ast_loader_result.modules, show_position=True
    )
    logging.info(result)
    return result
...
def test_categories_with_syntax_errors_abstract_syntax_tree(
    categories_with_syntax_errors_ast_loader_result: ASTLoaderResult,
    results_folder: Path,
):
    assert (
        categories_with_syntax_errors_ast_loader_result.diagnostics.has_errors()
...
def test_domains_abstract_syntax_tree(
...
def test_variables_abstract_syntax_tree(
...
def test_equations_abstract_syntax_tree(
