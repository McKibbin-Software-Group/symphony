...
@v_args(meta=True)
class AbstractSyntaxTreeTransformerOriginal(BaseTransformer):
    """
    ...
    def label(self, meta: Any, items: List[Token]) -> Tuple[str, SourcePosition]:
        position: SourcePosition = symphony_position(token)
        logging.debug(f"Processing label with children {items}")
        assert len(items) == 1, f"Expected one label but found {len(items)}"
        token = items[0]
        return ast.literal_eval(token.value), position

    def documentation(self, meta: Any, items: List[Token]) -> StringWithPosition:
        assert len(items) == 1
        token = items[0]
    ...
    ) -> Tuple[Optional[StringWithPosition], Optional[DimensionExpression]]:
    ...
    def category_declaration(self, meta: Any, items: List[Any]) -> AnyDeclaration:
        ...
        """
        position = symphony_position(meta)
    ...
    def dimension_declaration(self, meta: Any, items: List[Any]) -> AnyDeclaration:
        ...
        return self._build_dimension(
            position, name_token, label_text, label_pos, expression, documentation
        )
    ...
    def member_declaration(self, meta: Any, items: List[Any]) -> MemberDeclaration:
        """
        Helper for declarations with just NAME ":" label doc?.
        """
        position = symphony_position(meta)
    ...
    def dimensions_declaration(self, meta: Any, items: List[Any]) -> AnyDeclaration:
        type_position = symphony_position(meta)
        name_token: Token = items[0]
        label_text, label_pos = items[1]
        documentation, list_term = self._pick_doc_and_list(items[2:])
    ...
    def _build_dimension(
        ...
        """
        name_position = symphony_position(name_token)
