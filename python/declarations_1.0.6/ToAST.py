...
class ToAST(Transformer):
    ...
    Transformer additionally enforces:
      1) Every NAME listed in a dimension/category list must be a 'member' declared earlier.
      2) No member may appear in more than one category.
      3) Every member must be in a (single) category by the end of the file.
      4) Every member listed in a dimension must also belong to the SAME category declared earlier.
    """
    ...
    def decl(self, meta: Any, items: List[Any]) -> DeclNode:
        ...
        if decl_type in (DeclType.dimension, DeclType.category):
            ...
            if decl_type is DeclType.dimension:
                # NEW CONSTRAINT: All members in a dimension must also be in the SAME category.
    ...
    def start(self, meta: Any, items: List[DeclNode]) -> Program:
        # Final constraint (1): all declared members must be in a category
...
def main() -> int:
    cli = argparse.ArgumentParser(
        description=(
            "Parse declarations with semantic checks: "
