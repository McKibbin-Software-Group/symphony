...
def print_parse_error(err: UnexpectedInput, src: str, file_path: pathlib.Path) -> None:
    ...
    sys.stderr.write(f"\n❌ Parse error in {file_path} at line {line}, column {column}\n")
    if line_text:
        sys.stderr.write(f"   {line_text}\n")
        sys.stderr.write(f"   {_underline_column(line_text, column)}\n")

    # Show Lark's context if available
    try:
        context: str = err.get_context(src, span=60)
        sys.stderr.write(f"Context: {context.strip()}\n")
    except Exception:
        pass

    if isinstance(err, UnexpectedCharacters):
        allowed = sorted(getattr(err, "allowed", []))
        if allowed:
            sys.stderr.write(f"Unexpected character '{err.char}' — expected one of: {', '.join(allowed)}\n")
        else:
            sys.stderr.write(f"Unexpected character '{err.char}'\n")

        # Suggest close matches (helpful if user just misspelled something)
        if err.char.isalpha():
            suggestions = difflib.get_close_matches(err.char, allowed)
            if suggestions:
                sys.stderr.write(f"Did you mean: {', '.join(suggestions)}?\n")

    elif isinstance(err, UnexpectedToken):
        expected = sorted(err.expected)
        sys.stderr.write(f"Unexpected token {err.token!r}\n")
        sys.stderr.write(f"Expected one of: {', '.join(expected)}\n")
        # Suggest closest match for token type
        suggestions = difflib.get_close_matches(str(err.token.type), expected)
        if suggestions:
            sys.stderr.write(f"Closest match: {', '.join(suggestions)}\n")

    else:
        sys.stderr.write(f"{type(err).__name__}: {str(err)}\n")

    sys.stderr.write("Hint: Check for typos, missing ':' after names, or unclosed brackets.\n\n")
